...
export async function GET(request: Request) {
  ...
  if (!projectSlug) {
    return NextResponse.json({ error: 'project_slug requis' }, { status: 400 });
  }
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { id, project_slug, image, caption, sort_order } = body;

    if (!project_slug || !image) {
      return NextResponse.json({ error: 'project_slug et image requis' }, { status: 400 });
    }
