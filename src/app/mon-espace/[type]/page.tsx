...
const EMPTY_BON_CAISSE: Record<string, unknown> = {
  ...
  sens: "entree", montant: "", recu_de: "", montant_lettre: "", motif: "",
  avance: "", reste: "", ordre_donne_par: "", mode: "cash", depot_detail: "", compte: "entreprise",
};

const EMPTY_RECU: Record<string, unknown> = {
  date: new Date().toISOString().slice(0, 10),
  bpf: "", recu_de: "", somme: "", articles: "", avance: "", reste: "", etabli_par: "",
};

const EMPTY_BON_SORTIE: Record<string, unknown> = {
  date: new Date().toISOString().slice(0, 10),
  nom_emetteur: "", service: "",
...
const EMPTY_BY_TYPE: Record<string, Record<string, unknown>> = {
  bon_caisse: EMPTY_BON_CAISSE, recu: EMPTY_RECU, bon_sortie: EMPTY_BON_SORTIE,
};

export default function MonEspaceTypePage() {
  ...
  function openNew() { setEditing(null); setForm({ ...(EMPTY_BY_TYPE[docType] || {}) }); setError(""); }
  function openEdit(doc: GestionDoc) { setEditing(doc); setForm({ ...doc.data }); setError(""); }
  function closeForm() { setEditing(null); setForm({}); }
  const isFormOpen = editing !== null || Object.keys(form).length > 0;

  function setField(key: string, value: unknown) { setForm(prev => ({ ...prev, [key]: value })); }
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    const res = await fetch("/api/admin/gestion", {
      ...
      body: JSON.stringify({ id: editing?.id, doc_type: docType, data: form }),
    });
    if (res.ok) {
      setMsg(editing ? "Document mis à jour." : "Document créé.");
      setTimeout(() => setMsg(""), 3000); closeForm(); load();
  ...
  async function handleDelete(id: string) {
    ...
    await fetch(`/api/admin/gestion?id=${id}`, { method: "DELETE" }); load();
  }
  ...
  if (!config) {
    return (
      <GestionShell>
        ...
        <Link href="/mon-espace" className="btn-outline-sotip" style={{ textDecoration: "none" }}>Retour</Link>
      </GestionShell>
    );
  }
    ...
    <GestionShell>
      <div className="page-head">
        ...
        <div style={{ display: "flex", gap: ".5rem" }}>
          <Link href="/mon-espace" className="btn-outline-sotip" style={{ textDecoration: "none", padding: ".45rem 1rem", fontSize: ".85rem" }}>
      ...
      <div className="adm-card">
        <div className="adm-card-body" style={{ padding: 0, overflowX: "auto" }}>
          <table className="adm-table">
            <thead>
              <tr>
                <th>N° Document</th><th>Date</th><th>Référence / Bénéficiaire</th>
            ...
            <tbody>
                ...
                <tr key={doc.id}>
                  ...
                  <td>
                    <div style={{ display: "flex", gap: ".35rem" }}>
                      <Link href={`/mon-espace/imprimer/${doc.id}`} target="_blank" className="btn-sotip" style={{ padding: ".3rem .6rem", fontSize: ".75rem", textDecoration: "none" }} title="Imprimer / PDF">
              ...
              {!loading && docs.length === 0 && (
                <tr><td colSpan={5} style={{ textAlign: "center", padding: "2rem", color: "#6b7c8a" }}>Aucun document. Cliquez sur « Nouveau document » pour commencer.</td></tr>
              )}
              {loading && (
                            ...
                            item.type === "titre" ? (
                              <tr key={i} style={{ background: "#eef4d9" }}>
                                <td colSpan={4} style={{ border: "1px solid #e2e8f0", padding: ".2rem" }}>
                                  <input type="text" className="form-control" value={item.designation} onChange={e => setItem(i, "designation", e.target.value)} style={{ border: "none", fontWeight: 700 }} placeholder="Titre de section" />
                                </td>
