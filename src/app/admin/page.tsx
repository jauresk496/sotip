...
export default function AdminDashboardPage() {
  const [stats, setStats] = useState({ services: 0, projects: 0, gallery: 0, slides: 0, partners: 0, settings: 0 });
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    async function load() {
      try {
        const [sRes, pRes, gRes, slRes, ptRes, stRes] = await Promise.all([
  ...
  const cards = [
    ...
    { label: "Slides", count: stats.slides, icon: "bi-card-image", color: "#143c50" },
