...
export default function ProjectGalleryEditor({ projectSlug }: { projectSlug: string }) {
  ...
  async function load() {
    try {
      const res = await fetch(`/api/admin/project-images?project_slug=${projectSlug}`);
  ...
  async function handleReorder(id: string, direction: "up" | "down") {
    ...
    await Promise.all([
      fetch("/api/admin/project-images", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ id: item.id, project_slug: projectSlug, image: item.image, caption: item.caption, sort_order: swapItem.sort_order }),
      }),
      fetch("/api/admin/project-images", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ id: swapItem.id, project_slug: projectSlug, image: swapItem.image, caption: swapItem.caption, sort_order: item.sort_order }),
      }),
    ]);
    load();
  }
  ...
  return (
    <div style={{ borderTop: "1px solid #e2e8f0", paddingTop: "1rem", marginTop: "1rem" }}>
      <h4 style={{ fontSize: ".9rem", fontWeight: 700, color: "var(--ink)", marginBottom: ".8rem" }}>
        <i className="bi bi-images"></i> Galerie d'images du projet
      </h4>
